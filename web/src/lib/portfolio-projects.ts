...
export function nextPortfolioDeadline(projects: PortfolioProject[]): PortfolioProject | null { return [...projects].filter((project) => project.targetDate && !["published", "archived"].includes(project.status)).sort((a, b) => a.targetDate.localeCompare(b.targetDate))[0] || null; }
export function topPortfolioProject(projects: PortfolioProject[]): PortfolioProject | null { return [...projects].filter((project) => project.status !== "archived").sort((a, b) => Number(b.featured) - Number(a.featured) || portfolioProjectReadiness(b) - portfolioProjectReadiness(a))[0] || null; }
export function portfolioMilestoneRate(project: PortfolioProject): number { return project.milestoneCount ? Math.round((Math.min(project.completedMilestones, project.milestoneCount) / project.milestoneCount) * 100) : 0; }
export function portfolioRoleCounts(projects: PortfolioProject[]): Record<string, number> { return projects.filter((project) => project.targetRole && project.status !== "archived").reduce<Record<string, number>>((counts, project) => { counts[project.targetRole] = (counts[project.targetRole] || 0) + 1; return counts; }, {}); }
