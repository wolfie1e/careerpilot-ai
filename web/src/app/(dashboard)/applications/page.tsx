...
import { Archive, BriefcaseBusiness, CalendarClock, Download, ExternalLink, Mail, Plus, Search, Star, Trash2, Upload } from "lucide-react";
...
export default function ApplicationsPage() {
  ...
  function archiveClosedApplications() {
    setApplications((current) => current.map((application) => ["rejected", "withdrawn"].includes(application.stage) ? { ...application, archived: true } : application));
  ...
  function clearCompletedFollowUps() {
    setApplications((current) => current.map((application) => isApplicationFollowUpDue(application) ? { ...application, followUpAt: "" } : application));
  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button onClick={clearCompletedFollowUps} disabled={!followUpCount} className="rounded-xl border border-gray-700 px-3 text-sm font-medium text-gray-300 hover:text-white disabled:opacity-40">Clear due follow-ups</button>
        ...
        <button onClick={() => downloadJson("careerpilot-favorite-applications.json", favoriteApplications)} className="inline-flex items-center gap-2 rounded-xl border border-gray-700 px-3 text-sm font-medium text-gray-300 hover:text-white"><Star className="h-4 w-4" />Favorite export</button>
        <button onClick={() => downloadCsv("careerpilot-application-interviews.csv", interviewApplications.map((application) => ({ company: application.company, role: application.role, interview_at: application.interviewAt, contact: application.contactName, notes: application.notes })))} className="inline-flex items-center gap-2 rounded-xl border border-gray-700 px-3 text-sm font-medium text-gray-300 hover:text-white"><CalendarClock className="h-4 w-4" />Interviews</button>
      </div>
            ...
            <article key={application.id} className={cn("rounded-2xl border bg-gray-900 p-5", isApplicationFollowUpDue(application) ? "border-amber-500/50" : "border-gray-800")}>
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1 space-y-4">
                  ...
                  <input value={(application.tags || []).join(", ")} onChange={(event) => updateApplication(application.id, { tags: event.target.value.split(",").map((tag) => tag.trim()).filter(Boolean) })} placeholder="Tags, comma separated" className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-xs text-gray-300 outline-none" />
