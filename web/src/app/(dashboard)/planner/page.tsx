...
export default function PlannerPage() {
  ...
  function completeVisibleTasks() {
    ...
    setTasks((current) => current.map((task) => visibleIds.has(task.id) ? updatePlannerTaskStatus(task, "done") : task));
            ...
            <article key={task.id} className={cn("rounded-2xl border bg-gray-900 p-4", isPlannerTaskOverdue(task) ? "border-rose-500/40" : "border-gray-800")}>
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  ...
                  <input value={(task.tags || []).join(", ")} onChange={(event) => updateTask(task.id, { tags: event.target.value.split(",").map((tag) => tag.trim()).filter(Boolean) })} placeholder="Tags, comma separated" className="w-full rounded-xl border border-gray-800 bg-gray-950/50 px-3 py-2 text-sm text-gray-300 outline-none focus:border-blue-500" />
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <select aria-label={`Recurrence for ${task.title}`} value={task.recurrence || "none"} onChange={(event) => updateTask(task.id, { recurrence: event.target.value as PlannerTask["recurrence"] })} className="rounded-lg border border-gray-700 bg-gray-800 px-2.5 py-1.5 text-xs text-gray-300">
                      <option value="none">No repeat</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option>
                    </select>
